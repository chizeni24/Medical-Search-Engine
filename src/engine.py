...
df = read_data(os.path.join(parent_dir,"input/Dimension-covid.csv"))
df1=read_data(os.path.join(parent_dir, "input/Dimension-covid.csv"))
...
# change the directory to upper level 
os.chdir('..')
### Loading our pretrained model ###
skipgram = Word2Vec.load(os.path.join(os.getcwd(),'output/model_Skipgram.bin'))
FastText = Word2Vec.load(os.path.join(os.getcwd(),'output/model_Fasttext.bin'))
...
K1_abstract.to_csv(os.path.join(os.getcwd(),'output/skipgram-vec-abstract.csv'))

K1_title = return_embed(skipgram,df,"Title")
K1_title = pd.DataFrame(K1_title).transpose()  
K1_title.to_csv(os.path.join(os.getcwd(), 'output/skipgram-vec-title.csv'))
...
K2_abstract.to_csv(os.path.join(os.getcwd(),'output/FastText-vec-abstract.csv'))

K2_title = return_embed(FastText,df,"Title")
K2_title = pd.DataFrame(K1_title).transpose()    
K2_title.to_csv(os.path.join(os.getcwd(),'output/FastText-vec-title.csv'))

### Loading our pretrained vectors of each abstract ###
K = read_data(os.path.join(os.getcwd(),'output/skipgram-vec-abstract.csv'))   
